...
- **0.2.x** (legacy): 5-stage pipeline (summarizer → figma-scan → orienter → codegen → finalizer)
- **0.3.x** (current): 3-stage pipeline (index builder → unified codegen → finalizer)

**The 0.3.x pipeline is NOT backward compatible with 0.2.x artifacts or CLI flags.**

## Overview

Superconnect is a Node.js CLI (distributed as the `superconnect` npm package, Node >= 20) that runs a three‑stage pipeline:

1. **Index builder** – build searchable index of React/TypeScript or Angular repo components and exports
2. **Unified codegen** – agentic pipeline with tool access generates `.figma.tsx` (React) or `.figma.ts` (Angular) Code Connect files
3. **Finalizer** – summarizes the run and writes `figma.config.json` for Code Connect

The pipeline is orchestrated by `scripts/run-pipeline.js` and exposed as the `superconnect` binary.

Key architectural differences from 0.2.x:
- **Index-first approach**: Pre-built searchable index replaces filesystem exploration
- **Tool-enabled agents**: Agents use `queryIndex`, `readFile`, `listFiles` tools to explore codebase
...
- **Config file**: `superconnect.toml`
  ...
  - `[agent]` – `api`, `model`, `max_tokens`, `base_url`, `api_key`
    ...
    - `model` – model name (e.g., `"gpt-5.1-codex-mini"`, `"claude-haiku-4-5"`)
