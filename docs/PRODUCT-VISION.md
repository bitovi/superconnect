...
Superconnect is a command‑line tool that turns a Figma design system file and a React/TypeScript component repo into Figma Code Connect mappings.

It automates the pipeline from:
- Scanning a Figma file for component sets and variants
- Understanding the structure and exports of a React component library
- Deciding which source files implement which Figma components
- Emitting `.figma.tsx` Code Connect files and a `figma.config.json` that Figma can consume

## Problem it solves

- Creating and maintaining Code Connect mappings by hand is tedious and error‑prone.
- Design systems evolve quickly; keeping Figma and React in sync is hard.
...
Primary users:
- Design system engineers and front‑end engineers who own a React design system.
...
- Regenerate or update mappings safely when Figma or React components change.
