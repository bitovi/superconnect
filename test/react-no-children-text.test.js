...
const fixtureRoot = path.join(__dirname, '..', 'fixtures', 'react-no-children');

describe.skip('codegen gates implicit children/text from Figma STRING props (react)', () => {
  const runCodegen = () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'react-no-children-'));
  ...
  test('does not add children from label when component surface lacks text props', () => {
    const { files, contents } = runCodegen();
    expect(files).toEqual(['button.figma.tsx']);
    expect(contents).not.toContain("figma.string('label')");
    expect(contents).not.toContain('children: figma.string');
  });
});
