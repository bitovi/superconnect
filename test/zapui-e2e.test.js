...
maybeTest('agent SDK mode generates valid Code Connect', () => {
  ensurePrerequisites();
  const env = getEnvWithTokens();
  
  const subset = getOnlyList('ZAPUI_E2E_ONLY');
  if (subset?.length) {
    console.log(`ZapUI Agent SDK E2E (subset): ${subset.join(', ')}`);
  }
  ...
  try {
    // Setup with agent SDK mode
    copyFixture(tmpDir);
    writeConfig(tmpDir, FIGMA_URL, { agentApi: 'anthropic-agents', agentModel: 'claude-haiku-4-5' });
    fs.removeSync(path.join(tmpDir, 'superconnect'));
    fs.removeSync(path.join(tmpDir, 'codeConnect'));
